...
int _tmain(int argc,_TCHAR* argv[]) // OK
{
	...
	info.LoadingScreen = GetPrivateProfileInt("Custom", "LoadingScreen ", 0, ".\\MainInfo.ini");

	info.SSBarPosition = GetPrivateProfileInt("Custom", "SSBarPosition ", 0, ".\\MainInfo.ini");

	info.SBRegister = GetPrivateProfileInt("Custom", "SBRegister", 0, ".\\MainInfo.ini");

	info.loginbox99b = GetPrivateProfileInt("Custom", "loginbox99b", 0, ".\\MainInfo.ini");

	info.SSChar99b = GetPrivateProfileInt("Custom", "SSChar99b", 0, ".\\MainInfo.ini");
	...
	gCloak.Load("CustomCloak.txt");

	gCustomPosition.Load("CustomItemPosition.txt");

	gCustomRemake.Load("CustomEffectRemake.txt");

	gItemManager.Load("Item.txt");
...
#if(PREMIUN==1)
	HANDLE file = CreateFile("Premium/Licencia",GENERIC_WRITE,FILE_SHARE_READ,0,CREATE_ALWAYS,FILE_ATTRIBUTE_ARCHIVE,0);
#else
