...
CMinimap::CMinimap()
{
	...
	this->Moving = false;
	this->ultimoX = 535;//Example of initial coordinate {X}. Coordinate that will be constantly modified.
...
void Interface::DrawMiniMap()
{
	...
	//-- Usuario
	if ( gProtect.m_MainInfo.CustomInterfaceType == 1 || gProtect.m_MainInfo.CustomInterfaceType == 2 || gProtect.m_MainInfo.CustomInterfaceType == 5)
	...
	if(TimerBar != 0)
	{
	//RenderBitmap(31458, InitialX + ReturnX(TransForX(3)), InitialY + ReturnY(TransForY(71)), ReturnX(TransForX(90)), ReturnY(TransForY(44)), -0.4, -0.4, 1.2, 1.2, 0.1, 1, 0); //Minimapa v2 Timer
	...
	//--
	pSetBlend(true);
	this->DrawFormat(eWhite, InitialX + ReturnX(TransForX(26)), InitialY + ReturnY(TransForY(85)), ReturnX(TransForX(100)), ReturnY(TransForY(1)), LocalTime);
	pGLSwitchBlend();
	pGLSwitch();
	pSetBlend(false);
	}
	...
	if (pCheckMouseOver(x1, y1, w1, h1))
	{
		this->DrawToolTip(v11 + 26, v10 - 20, "Jewel of Bank");
		if (*(BYTE*)0x8793386)
		{
			PlayBuffer(25, 0, 0);
...
void Interface::MiniMapMini()
{
	...
	else if (gProtect.m_MainInfo.CustomInterfaceType == 0 || gProtect.m_MainInfo.CustomInterfaceType == 1 || gProtect.m_MainInfo.CustomInterfaceType == 2 || gProtect.m_MainInfo.CustomInterfaceType == 5)
	{
	int y = InitialY - 5;
	int x = InitialX + 87;
