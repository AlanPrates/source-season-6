...
//--
	...
	//-> Loading Screen
	if (gProtect.m_MainInfo.LoadingScreen == 1)
	{
		gCUIMng.Load();
	}

	//-> Select Server Bar Position
	if (gProtect.m_MainInfo.SSBarPosition == 1)
	{
		g_ServerSelWin.Init();
	}

	//-> Auto Login System
	if (gProtect.m_MainInfo.SBRegister == 1)
	{
	AutoLogin.Load();
	...
	//-> SelectChar+SelectServer 99b
	if (gProtect.m_MainInfo.SSChar99b == 1)
	{
		gCSceneA.Load();
	}
	//--
	...
	//-> Login Remake 99b
	if (gProtect.m_MainInfo.loginbox99b == 1)
	{
		RemakeLoginLoad();
	}
	//--
